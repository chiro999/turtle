...
void close(input_t *input_variables)
{
    ...
    while (input_variables->tokens[index] != NULL)
    {
        if (_strncmp(input_variables->tokens[index], "exit") == 0 && input_variables->tokens[index + 1] != NULL)
        {
            index++; // Move to the next token
            close_status = _atoi(input_variables->tokens[index]); /* Convert the exit status string to an integer */
            ...
            if (close_status == -1)
            {
                ...
                str_out(input_variables->tokens[index]); /* Print the problematic input */
